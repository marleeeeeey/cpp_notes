...
class talk_to_svr : public boost::enable_shared_from_this<talk_to_svr>, public coroutine, boost::noncopyable
{
    ...
    talk_to_svr(const std::string& username) : sock_(service), started_(false), username_(username), timer_(service) {}
    void start(ip::tcp::endpoint ep) { sock_.async_connect(ep, MEM_FN2(step, _1, 0)); }
public:
