...
### `boost::intrusive_ptr` - интрузивный подсчёт ссылок

- Проблема `shared_ptr` в том, что потоки серриализуются на атомарном счетчике ссылок.
- В `boost::intrusive_ptr` - нет необходимости хранить счетчик ссылок внутри указателя.
- Объект, к которому он обращается, должен содержать счетчик ссылок внутри себя и иметь методы управления этим счетчиком.
- Счетчик ссылок по дефолту может быть не атомарным, если мы знаем, что у нас нет многопоточности.
...
public:
    ...
    friend void intrusive_ptr_add_ref(MyObject* obj) {
        ++obj->ref_count;
    }

    friend void intrusive_ptr_release(MyObject* obj) {
        if (--obj->ref_count == 0) {
            delete obj;
        }
    }
...
int main() {
    ...
    {
        boost::intrusive_ptr<MyObject> ptr2 = ptr1;         // <=== Копирование указателя увеличивает счетчик ссылок
        std::cout << "Inside the inner scope.\n";
    }                                                       // <=== Уничтожение ptr2 уменьшает счетчик ссылок
