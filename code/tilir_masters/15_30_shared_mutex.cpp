...
struct Unique
{
    ...
    int get() const
    {
        std::unique_lock<std::mutex> lock{m};       // <=== BAD: Тратим массу ценного времени на блокировку (дорогой лок)
        return value;
    }
...
struct Shared
{
    ...
    int get() const
    {
        std::shared_lock<std::shared_mutex> lock{m};    // <=== GOOD: Тут мы не всегда делаем блокировку. Только если unique_lock уже заблокирован
        return value;
    }

    void modify(int newval)
    {
        std::unique_lock<std::shared_mutex> lock{m};    // <=== std::unique_lock
        value = newval;
    }
};
...
void modifier(T& t)
{
    for (int i = 0; i < nmod; i += 1)
    {
        if ((i % 1000) != 0)
            continue;
        t.modify(i);
        noopt(i);                           // <=== noopt - чтобы компилятор не оптимизировал код
    }
}
