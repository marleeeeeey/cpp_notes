...
- `std::scoped_lock` (C++17) - RAII, который лочит сразу несколько мьютексов в правильном порядке. Отпускает в деструкторе. Заменяет `std::lock` + `std::lock_guard`. Но он тяжелее `std::lock_guard`.
