...
// Класс, который на каждом пятом элементе бросает исключения.
struct Controllable
{
    static int control;
    int* resource_;
...
// Буфер, который делает операции с сырой памятью.
template <typename T>
struct MyVectorBuf
{
protected:
...
protected:
    ...
    MyVectorBuf(MyVectorBuf&& rhs) noexcept : buffer_(rhs.buffer_), capacity_(rhs.capacity_), size_(rhs.size_)
    {
        rhs.buffer_ = nullptr;
        rhs.capacity_ = 0;
        rhs.size_ = 0;
    ...
    MyVectorBuf(size_t sz = 0)
      : buffer_((sz == 0) ? nullptr : static_cast<T*>(::operator new(sizeof(T) * sz))), capacity_(sz)
    {}
...
// Безопасный к исключениям вектор.
template <typename T>
