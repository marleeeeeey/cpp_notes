...
class proxy : public boost::enable_shared_from_this<proxy>, boost::noncopyable
{
    ...
    proxy(ip::tcp::endpoint ep_client, ip::tcp::endpoint ep_server) : client_(service), server_(service), started_(0) {}
...
private:
    ...
    {
        async_read(
            sock, buffer(buff, max_msg), MEM_FN3(read_complete, std::ref(sock), _1, _2),
    ...
    void do_write(ip::tcp::socket& sock, char* buff, size_t size)
    {
        if (!started())
            return;
        sock.async_write_some(buffer(buff, size), MEM_FN3(on_write, std::ref(sock), _1, _2));
    }
