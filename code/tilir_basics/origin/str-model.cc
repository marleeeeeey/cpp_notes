...
class Drawable
{
    ...
    struct DrawableInt final : IDrawable
    {
        ...
        std::unique_ptr<IDrawable> copy_() const override { return std::make_unique<DrawableInt>(*this); }
        void draw_(Screen& out, size_t position) const override { ::draw(data_, out, position); }
    };
    ...
    struct DrawableVector3D final : IDrawable
    {
        ...
        std::unique_ptr<IDrawable> copy_() const override { return std::make_unique<DrawableVector3D>(*this); }
        void draw_(Screen& out, size_t position) const override { ::draw(data_, out, position); }
    };
...
public:
    friend void draw(const Drawable& x, Screen& out, size_t position) { x.self_->draw_(out, position); }
};
