...
- [13.1 Определители типов и SFINAE](#131-определители-типов-и-sfinae)
...
## 12.2 Разрешение имен в шаблонах
...
## 13.1 Определители типов и SFINAE
...
- Представим, что конструктор заблокирован, а нам нужно оценить тип возвращаемого значения метода объекта, который мы не можем создать.
  ...
  - Как решение в стиле С мы могли бы скастить `void*` к нужному типу и через `->` обратиться к методу. Но, к счатью, это UB.
- `declval<T>()` - функция без тела, которая возвращает `T&&`.
  - `template <typename T> add_rvalue_reference_t<T> declval();`
  - Она только объявлена, но не определена. Поэтому, если мы попытаемся вызвать её, то получим ошибку линковки.
  - Она позволяет нам получить тип возвращаемого значения метода объекта `T`, не создавая объект `T`.
...
#endif
    decltype(std::declval<Foo>().bar()) a; // OK
    (void)a;
}
```
...
- `Run Time` - столбец означает, что НУЖНЫ ВЫЧИСЛЕНИЯ В RUNTIME.
- `Type` - строка означает, что НУЖНЫ ПРЕОБРАЗОВАНИЯ НАД ТИПАМИ.

- Квадрант 3 - это и есть метапрограммирование.
