...
- `test -z string` - проверка строки на пустоту.
  ...
  - `-n` - проверка строки на непустоту.
  - `string1 = string2` - проверка строк на равенство.
  - `string1 != string2` - проверка строк на неравенство.
  - `string1 > string2` - проверка строк на лексикографический порядок.

#### `test` или `[ ]` для ЦЕЛЫХ ЧИСЕЛ

- `test 5 -eq 5` - проверка чисел на равенство.
  - `-eq` - равно.
  - `-ne` - не равно.
  - `-lt` - меньше.
  - `-le` - меньше или равно.
...
#### Более современная версия `test` - `[[ ]]`

- Добавляет выражение для проверки строк на равенство регулярному выражению и wildcard-символы.
- `[[ "$INT" =~ ^[0-9]+$ ]]` - проверка на число.
  - `=~` - регулярное выражение.
- `[[ "$FILE" == foo.* ]]` - проверка на совпадение строки.
  - `==` - совпадение строки через wildcards.

#### `(())` проверка целых чисел

- Может применятся к целым числам. Распознает переменные по именам и не требует выполнять подстановку.
- `((1))` - вернет 0, т.е. для if оператора это прохождение проверки.
  - `if ((1)); then echo "ok"; else echo "nok"; fi` - выводит `ok`.
  - `if ((0)); then echo "ok"; else echo "nok"; fi` - выводит `nok`.

#### Объединение выражений с помощью логических операторов

- Используйте круглые скобки для группировки выражений.
- Поскольку все выражения и операторы в команде `test` интерпретируются командной оболочкой как аргументы, в отличие от `[[ ]]` И `(( ))`, символы, имеющие специальное значение для оболочки (`<, >, (, )`) необходимо заключать в кавычки или экранировать.
- Операторы логического И и ИЛИ могут быть использованы для ветвления.
  - Если первое условие истинно, то второе условие не проверяется для `&&`.
  - Если первое условие ложно, то второе условие не проверяется для `||`.
  - `mkdir temp && cd temp` - создание директории и переход в нее.
  - `[ -d temp ] || mkdir temp` - проверка существования директории и создание ее, если ее нет.
...
- Примеры
  - `if [[ INT -ge MIN_VAL && INT -le MAX_VAL ]]; then ... fi` - проверка на диапазон чисел.
  - `if [ -f "$FILE" -a -r "$FILE" ]; then ... fi` - проверка на существование файла и доступ на чтение.

### Глава 28. Чтение ввода с клавиатуры

#### `read` - чтение ввода с клавиатуры

- `read var` - чтение ввода с клавиатуры.
- `read -p "Enter a number: " var` - вывод приглашения перед вводом.
- `read -n 1 var` - чтение одного символа без нажатия Enter.
- `read -s var` - чтение ввода без отображения вводимых символов.
- `read -t 10 var` - чтение ввода с таймаутом в 10 секунд.
- `read -a array` - чтение ввода в массив. Затем `echo ${array[0]}`.
- `read var1 var2 var3` - чтение ввода в несколько переменных.
- `read` нельзя использовать в пайпе, потому что он читает из стандартного ввода.
  - `echo "yes" | read var` - не работает.
  - Потому что `read` выполняется в отдельном процессе (отдельной оболочке). И после завершения процесса переменная `var` удаляется.

### Глава 29. Управление потоком выполнения: циклы while и until

#### `while` - цикл с предусловием

- `break` and `continue` - прерывание и продолжение выполнения цикла.

```bash
a=0
while [ $a -le 5 ]; do # [[ $a != 5 ]] # <=== MODEERN WAY
        echo Hello $a
        a=$((a+1))
done
echo Finished
...
#### `until` - цикл с инвертированным предусловием

- `until` - похож на `while`, но по сути условие инвертировано.

#### Чтение файла построчно в цикле

- `while read line; do ... done < file.txt` - чтение файла построчно.
  - `while read v1 v2 v3 v4 v5 v6; do printf "hello: %s  %s  %s  %s  %s  %s\n" $v1 $v2 $v3 $v4 $v5 $v6; done < sidr.txt`
  - Такое же можно сделать с использование конвейера:
    - `cat sidr.txt | while read v1 v2 v3 v4 v5 v6; do printf "hello: %s  %s  %s  %s  %s  %s\n" $v1 $v2 $v3 $v4 $v5 $v6; done`.

### Глава 30. Поиск и устранение ошибок

- Синтаксические ошибки
- Логические ошибки
- Проверяйте коды завершения программ и команд, используемых сценарием: `$?`.
- Пишите конструкции вида `cmd1 && cmd2`. Если `cmd1` завершилась с ошибкой, то `cmd2` не выполнится.
- Тщательно проверяйте ввод пользователя.
- Тестируйте.
- "Выпускай раньше, выпускай чаще" - Софт, выпускаемый раньше, получает больше времени на использование и тестирование. Опыт показывает, что ошибки тем легче находить и дешевле исправить, чем раньше в цикле разработки они будут обнаружены.
- Пишите заглушки в коде. Вместо `rm *` напишите `echo rm *`.
- Тестирование есть функция времени - не каждую особенность сценария нужно тщательно тестировать.
- Для сужения области поиска ошибок комментируйте части кода.
- При отладке используйте `set -x` и `set +x` для вывода отладочной информации. Можно обернуть в эти команды части скрипта, которые хочется отладить.
  - `set -x` - включение отладочной информации.
  - `set +x` - выключение отладочной информации.
  - Можно также в заголовке сценария добавить `#!/bin/bash -x`.
...
- Шаблоны могут содержать метасимволы: `*`, `?`, `[...]`.
- Несколько шаблонов могут быть объединены в один с помощью `|`.
...
- `shift` - сдвиг параметров. `$1` становится `$2`, `$2` становится `$3`, и так далее. При этом `$#` уменьшается на 1.
...
- `$FUNCNAME` - имя функции. Оболочка автоматически присваивает значение этой переменной в момент вызова функции.
  - `$1`, `$2`, ... - позиционные параметры функции.
  - `$0` - Всегда содержит имя сценария, даже если он вызван из функции.
...
- `"$@"` - все параметры в виде списка, где каждый параметр заключен в кавычки.
- `$*` - все параметры в виде строки.
- `"$*"` - все параметры в виде строки, где весь скоуп параметров заключен в кавычки.
...
#### Элегантное решение парсинга параметров командной строки

- Цикл продолжается, пока позиционный параметр `$1` не станет пустым.
- В конце цикла параметр `$1` сдвигается на одну позицию.
...
- `for var in word1 word2; do echo $var; done` - цикл for с перечислением слов.
- `for var in {1..5}; do echo $var; done` - цикл for с диапазоном.
- `for var in file*.txt; do echo $var; done` - цикл for с шаблоном.
  - Если файлов нет, то шаблон остается неизменным и выводится как есть.
- `for var; do echo $var; done` - Если не указан диапазон итерации, то по умолчанию обрабатываются позиционные параметры.
- `for ((i=0; i<5; i++)); do echo $i; done` - цикл for с арифметическим выражением.

### Глава 34. Строки и числа

#### Фигурные скобки `{}` для отделения переменных от текста

- `{}` - позволяют отделить переменные от текста: `${var}1`.

#### Подстановка пустых параметров `$var:-default`

- `${var:-default}` - подстановка значения по умолчанию, если переменная не определена или пуста.

#### `:?` - проверка на пустое значение и завершение с ошибкой

- `${var:?"error message"}` - проверка на пустое значение и завершение с ошибкой.

#### Получение имен переменных `${!prefix*}`

- `${!prefix*}` - получение имен переменных, начинающихся с префикса.

#### Подсчет количества символов `${#var}`

- `${#var}` - подсчет количества символов в строке.

#### Извлечение подстроки `${var:offset:length}`

- `${var:offset:length}` - извлечение подстроки.

#### Удаление с начала `${var#pattern}` и с конца `${var%pattern}`

- `${var#pattern}` - удаление с начала.
- `${var%pattern}` - удаление с конца.

#### Поиск с заменой `${var/pattern/replacement}`

- `${var/pattern/replacement}` - поиск и замена первого вхождения.
- `${var//pattern/replacement}` - поиск и замена всех вхождений.
- `${var/#pattern/replacement}` - поиск и замена в начале строки.
- `${var/%pattern/replacement}` - поиск и замена в конце строки.

#### Системы счисления `$((0x10))` или `$((2#1010))`

- `$((0x10))` - шестнадцатеричное число.
- `$((2#1010))` - двоичное число.
- `$((0#12))` - восьмеричное число.

#### Арифметические операции `$((2 + 2))`, `$((2 * 2))`, `$((2 / 2))`, `$((2 % 2))`
...
- `echo $((2 & 2))` - побитовое И.
- `echo $((2 | 2))` - побитовое ИЛИ.
- `echo $((2 ^ 2))` - побитовое исключающее ИЛИ.
- `echo $((~2))` - побитовое НЕ.
- `echo $((2 << 1))` - сдвиг влево.
- `echo $((2 >> 1))` - сдвиг вправо.

#### Логические операторы `&&`, `||`, `!=`, `==`, `>`, `<`, `>=`, `<=`, `ex1?ex2:ex3`

- `echo $((2 && 2))` - логическое И.
- `echo $((2 || 2))` - логическое ИЛИ.
- `echo $((2 != 2))` - не равно.
- `echo $((2 == 2))` - равно.
- `echo $((2 > 2))` - больше.
- `echo $((2 < 2))` - меньше.
- `echo $((2 >= 2))` - больше или равно.
- `echo $((2 <= 2))` - меньше или равно.
- `echo $((2 > 2 ? 1 : 0))` - тернарный оператор.

#### `bc` - калькулятор с произвольной точностью

- `echo "2.6 + 2" | bc` - сложение c произвольной точностью.
...
var=123
echo ${var}1        # 1231
echo $var1          # <Empty> - var1 is not defined

echo ${var:-456}    # 123 - var is defined
echo ${bar:-456}    # 456 - bar is not defined. Show default value.

echo ${!BASH*}      # BASH BASHOPTS BASHPID ...

echo ${#var}        # 3 - length of var

echo ${var:1:2}     # 23 - substring from 1 to 2

big="bigstring"
echo ${big#big}         # string - remove big from the beginning
echo ${big%string}      # big - remove string from the end
echo ${big/big/small}   # smallstring - replace first big with small

echo $((0x10))      # 16 - hexadecimal
...
echo ${var:?"var is not defined"}           # 123 - var is defined
echo ${bar:?"bar is not defined. ERROR"}    # Error: bar is not defined
```

### Глава 35. Массивы

- Оболочка поддерживает массивы в огранниченной форме.
- `array=(1 2 3 4 5)` - создание массива.
- `array[0]=1` - присваивание элементу массива.
- `echo ${array[0]}` - вывод элемента массива. Скобки обязательны.
- `echo ${array[*]}` - вывод всех элементов массива.
- `echo ${array[@]}` - вывод всех элементов массива. Идентично `*`, пока не используется в кавычках.
- `echo ${#array[*]}` - количество элементов массива.

```bash
foo=([2]=a [4]=b [6]=c)
for i in "${foo[@]}"; do echo $i; done # a b c  - values
for i in "${!foo[@]}"; do echo $i; done # 2 4 6 - indexes

foo=(a b c)
echo ${foo[@]}
foo+=(d e f)        # append to array
echo ${foo[@]}
foo=A               # Set first element of array to A
echo ${foo[@]}

unset foo           # delete array
unset `foo[2]`        # delete element. Quotes are required to prevent substitution.
```
...
- `{ command1; command2; }` - **группы команд**.
  - Все команды входящие в группу, **выполняются в текущей оболочке**.
  - Выполняются быстрее и требуют меньше памяти, чем подоболочки.
  - Фигурные скобки должны быть разделены пробелами от команд.
  - После последней команды должна быть точка с запятой.
- `(command1; command2)` - **подоболочка**.
  - Все команды входящие в подоболочку **выполняются в дочерней копии** текущей оболочки.
- `{ ls -l; echo "Listing of foo.txt"; cat foo.txt; } > output.txt` - вывод в файл.
  - Каждая команда в группе будет перенаправлена в файл.
- `{ ls -l; echo "Listing of foo.txt"; cat foo.txt; } | lpr` - вывод всех команд в группе в принтер.

#### Подстановка процессов

- Все команды входящие в группу, **выполняются в текущей оболочке**: `{ command1; command2; }`.
- Все команды входящие в подоболочку **выполняются в дочерней копии** текущей оболочки: `(command1; command2)`.
- **Конвейеры команда всегда выполняются в подоболочке: `command1 | command2`**.
- Можно интерпретировать вывод оболочки как обычный файл: `echo <(echo "Hello")`.
  - Выведет `/dev/fd/63`. Потому что вывод подоболочки передается через файловый дескриптор.

#### `trap` - перехват сигналов

- `trap arguments signals` - перехват сигналов. arguments - команды, которые будут выполнены при получении сигнала.
- `trap "echo 'Caught signal'" SIGINT` - перехват сигнала.
- `trap "echo 'I am ignoring you.'" SIGINT SIGTERM` - перехват нескольких сигналов.
- `trap custom_function SIGINT` - перехват сигнала и вызов пользовательской функции.

#### Временные файлы `mktemp`

- `mktemp` - создание временного файла.
- `mktemp /tmp/tempfile.XXXXXX` - создание временного файла с шаблоном.

#### Асинхронное выполнение дочерних сценариев: `&` and `wait`

- `&` - запуск дочернего процесса в фоне.
...
- `mkfifo fifo` - создание именованного канала.
- `process1 > fifo &` - запись в канал.
- `process2 < fifo` - чтение из канала.
- `process1 | process2` - аналогично предыдущим двум пунктам.
