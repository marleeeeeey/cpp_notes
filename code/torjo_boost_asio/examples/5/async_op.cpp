...
private:
    ...
    struct operation
    {
        operation(io_service& service, op_func op, completion_func completion)
          : service(&service), op(op), completion(completion), work(new io_service::work(service))
        {}
        operation() : service(0) {}
        io_service* service;
        op_func op;
...
boost::system::error_code compute_file_checksum(std::string file_name)
{
    HANDLE file = ::CreateFile(
        file_name.c_str(), GENERIC_READ, 0, 0, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, 0);
...
int main(int argc, char* argv[])
{
    ...
    async_op::new_()->add(boost::bind(compute_file_checksum, fn), boost::bind(on_checksum, fn, _1), service);
    service.run();
}
